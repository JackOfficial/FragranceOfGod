...
<section class="content">
    <div class="container-fluid">
        <div class="card shadow-sm" x-data="{ imagePreviews: @json($story->media->where('file_type','image')->pluck('file_path')), docs: [] }">
            <div class="card-header bg-gradient-primary text-white">
                <h3 class="card-title"><i class="fas fa-newspaper"></i> Edit Story</h3>
            </div>
            ...
            <div class="card-body">
                ...
                <form action="{{ route('admin.stories.update', $story->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <fieldset class="border p-3 mb-3">
                        <legend class="w-auto px-2"><i class="fas fa-info-circle"></i> Story Info</legend>
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="title"><i class="fas fa-heading"></i> Story Title <span class="text-danger">*</span></label>
                                    <input type="text" name="title" id="title" class="form-control" value="{{ old('title', $story->title) }}" required>
                                </div>
                            </div>

                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="is_published"><i class="fas fa-eye"></i> Publish Story?</label>
                                    <select name="is_published" id="is_published" class="form-control">
                                        <option value="1" {{ old('is_published', $story->is_published) == 1 ? 'selected' : '' }}>Yes</option>
                                        <option value="0" {{ old('is_published', $story->is_published) == 0 ? 'selected' : '' }}>No</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                    ...
                    <fieldset class="border p-3 mb-3">
                        ...
                        <div class="form-group">
                            <textarea name="description" id="description" class="form-control summernote">{{ old('description', $story->description) }}</textarea>
                        </div>
                    </fieldset>
                    ...
                    <fieldset class="border p-3 mb-3">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="images"><i class="fas fa-image"></i> Story Images</label>
                                    ...
                                    <template x-if="imagePreviews.length">
                                        <div class="mt-2 d-flex flex-wrap gap-2">
                                            <template x-for="src in imagePreviews" :key="src">
                                                <img :src="src.includes('storage/') ? '{{ asset('storage') }}/' + src : src" class="img-thumbnail" style="width:100px; height:100px; object-fit:cover;">
                                            </template>
                                        </div>
                                    </template>
                                </div>
...
<script>
    $(document).ready(function() {
        $('.summernote').summernote({
            placeholder: 'Write detailed story description here...',
            tabsize: 2,
            height: 180
        });
    });
